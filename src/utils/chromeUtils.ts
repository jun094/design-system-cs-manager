 ...
 */
export const safeTabsQuery = async (queryInfo: chrome.tabs.QueryInfo): Promise<chrome.tabs.Tab[]> => {
 ...
 */
export const safeCaptureVisibleTab = async (): Promise<string | null> => {
  if (!isChromeExtension()) {
    console.warn('크롬 확장 프로그램 환경이 아닙니다.');
    return null;
  }
 ...
 */
export const safeExecuteScript = async (
  tabId: number, 
  func: () => any
): Promise<any> => {
  if (!isChromeExtension()) {
